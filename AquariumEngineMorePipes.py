...
class Aquarium():
    def __init__(self, width=580, height=520, n_pipes=3):
        ## initialise pygame
        pygame.init()
        pygame.font.init()
        ...
        ## pipes
        self.pipes = [Pipe(self.width/3), Pipe(self.width*2/3)]
    ...
    def game_loop(self):
        while self.game_running:
            ...
            # Move food down pipe
            for pipe in self.pipes:
                pipe.move_food(boundary_y=self.height)
                self.score += pipe.check_for_eaten(self.player)
